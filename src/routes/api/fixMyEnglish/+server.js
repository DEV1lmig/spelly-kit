import { TextServiceClient } from '@google-cloud/text-to-speech';
...
export const GET = async ({ url }) => {
  ...
  const client = new TextServiceClient({
    credentials: JSON.parse(SECRET_GEMINI_API_KEY)
  });

  const geminiGeneration = await client.generateText({
    model: 'text-bison-001',
    prompt: {
      text: `this is a spell check generator. It can't recieve offensive languege, (explicit racism, xenofobia and religion. Anything else its fine.)
--
...
Correct sample:`
  ...
  console.log(geminiGeneration[0])
  if (geminiGeneration[0].candidates) {
    const geminiResponseText = geminiGeneration[0].candidates[0].output
    const formattedResponse = geminiResponseText.replace('--', '').replaceAll('"', '').trim()
